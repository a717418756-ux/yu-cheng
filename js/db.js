...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.7.4';     // 修正嚴重資料損毀:題目答案會被無聲改成 'A'。根因為四處硬編碼預設值—①showAdd 開啟編輯時 `S.correct = q.answer ? ... : new Set(['A'])`，題目 answer 為空就被當成選了A，一存檔即寫死 ②_getAnswerStr 的 `return S.correct||'A'` 在狀態未初始化時憑空產生A ③setQT 切換題型時強制補A ④表單重置時預設A。四處全部改為「留空不預設」，未選答案時如實存空字串並提示使用者(仍允許存檔，這種題目答題時不計入複習進度與統計)。註：雲端還原本身是原樣 bulkPut 未做轉換，損毀發生在還原後開啟編輯畫面並存檔的環節
