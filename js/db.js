...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.8.1';     // ①修申論答案卷格線越來越偏:padding-top(14px)與background-position的Y偏移(6px)不一致，第一行就錯開8px；且 .l1 有 margin-top:2px，每個大標都讓後續行再累積偏移。改為兩者都歸零、格線間距完全等於行高34px、移除margin-top(區隔改靠字重不動高度) ②題庫篩選標籤加上題數顯示(選擇/申論/錯題/收藏/各科目)，0題者反灰並停用(pointer-events:none)；題數依當前科目與關鍵字連動計算，切換類型才有意義；目前選中的類型即使為0仍保持可按，避免卡住無法切回
