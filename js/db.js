...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.15.3';    // ①修官網鈕與刪除鈕太近會誤觸(刪除不可逆，風險高):官網鈕從右側箭頭旁移到左側資訊區下方，改為「🏛 官網原文」文字鈕，與右側🗑徹底分開 ②卡片點擊判斷從 classList.contains('lw-del') 改為 closest('.lw-del, .lw-gov')—原寫法在按鈕內含文字時，點到文字節點的 e.target 不是按鈕本身會判斷失敗；雖然 lw-gov 已有 stopPropagation 擋住，但兩處邏輯不一致是隱患，一併補齊。今日新增全面掃描:死碼零殘留(舊正則_CITE_*、law-official、backHtml、Google fallback 皆已清除)，10個新函式全部有使用，四層篩選/模擬考/批次修改/掌握度回歸正常
