...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.20.3';    // 換章後白按與往回停在倒數第二頁(使用者記錄 1/4→3/4):實測章節剛顯示時 epub.js 已算出總頁數，但內容寬度要晚 0.1～0.5 秒才撐開到那麼寬，這段期間捲不過去或被卡住。①新增 _epubReady 等內容寬度達到總頁數才翻頁/定位 ②翻頁進行中再按會排隊，不再同時跑兩個 ③_epubToSectionEnd 改為內容撐開後一次算準 ④移除 default manager 不存在的 fill() 呼叫(死碼) ⑤診斷記錄加入捲動位置、內容寬、每頁寬、等待時間
