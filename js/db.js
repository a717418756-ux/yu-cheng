...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.1.7';     // ①返回鍵重新設計：改用一次性預先推入60筆history緩衝(取代原本動態setTimeout補回)，消除補回時的時序空窗—原本延遲補回若被系統搶先判定「無更多歷史」會直接關閉App，完全繞過JS攔截，這是「不穩定、有時連首頁都沒到就退出」的根因；新設計下逐層回退與離開流程全交由瀏覽器原生back()鏈完成，不再有setTimeout。用真實history語意(pushState截斷前進紀錄)+實際擷取程式碼端對端驗證：3層深度需按3次逐層回首頁，第4次於首頁時連續消耗剩餘緩衝觸發真正離開，9種既有情境同時複測全過 ②修法條連結對不上資料的根因：showLawPop自製簡陋正則算出的artNum是原始數字(如92)，但articleNumber是art2n()算出的「主號×1000+子號」格式(如92000/92004)，量級差1000倍導致比對永遠失敗；改為直接呼叫art2n(ref)取得同單位數值，namePart改用「第X條」出現位置切割而非到處刪數字，同時修正「法規名稱含數字被誤判為條號」的原始bug。此問題不因重建索引才發生—重建索引前資料庫若殘留舊格式(未乘1000)articleNumber會與舊邏輯巧合對上，重建後統一為正確格式反而讓舊邏輯完全對不上，此次修正後兩者永久保持一致，用真實擷取的比對邏輯端對端驗證含子條號情境全部正確
