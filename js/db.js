...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.19.2';    // 電子書跳頁的真正原因:display(CFI) 只保證停在「該段文字所在的頁」，該段若從前面幾頁接下來就會停在前面(實測差到13頁)。開書還原位置、調字級還原都用它，所以一開書或一調字級就少讀了好幾頁。改為定位後比對 CFI，目標還在後面就補翻到目標所在頁。同時移除 4.19.1 用 display 做的跨章修正(它本身就會造成跳頁)，翻頁回到 epub.js 原生 next/prev(實測跨章、雙欄、錨點目錄都對稱)
