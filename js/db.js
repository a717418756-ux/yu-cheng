...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.1.4';     // 自我檢查修正:①結束/離開測驗前先保存未存的標註—原本_inkTeardown直接清空並重設dirty，畫完沒按完成就按結束會導致筆跡永久遺失 ②標註模式原touch-action:none使長題目完全無法捲動，改pan-y並改為手指長按260ms才進入繪製(移動即取消)，觸控筆/滑鼠仍直接繪製不受影響 ③返回鍵漏掉測驗新增的彈窗—科目多選與備註用的是.cov不是.ov，掃描條件補上.cov.on並加入對應關閉函式，否則按返回會略過它們去關更底層的東西(cfm仍在第1層優先處理，不衝突)
