...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.1.5';     // ①修單選題(ansQ)公布答案不顯示關聯法條的舊bug(v3.6.6即存在，非本次改動造成)—showQLawLinks原本只有多選/申論路徑呼叫，單選兩個分支都補上 ②修申論題在渲染新題目時就提前呼叫showQLawLinks，等於未作答就曝光法條線索；改為渲染時隱藏、revealES公布解答時才顯示，與選擇題一致 ③返回鍵新增「標註模式優先退出」層—原本標註中按返回會直接跳出整個答題畫面，現在會先退出標註模式(自動保存筆跡)，符合逐層遞減的直覺
