...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.19.4';    // 電子書翻頁比照專業閱讀器:①圖片/表格一律限縮在版面內(超寬會讓分欄算錯，之後每次重排都位移) ②epub.js 的捲動補償(counter)只在使用者操作當下生效：圖片載入完、內容慢慢排完也會觸發它，補償量卻用舊尺寸算，就會莫名跳好幾頁 ③實體翻頁鍵/方向鍵接管(Boox側鍵、藍牙翻頁器)，不接管會被瀏覽器拿去捲動外層 ④觸控區 touch-action:manipulation、外層禁止回彈捲動 ⑤往前翻的操作視窗拉長到1.5秒(接回上一章需要排版時間) 另保留位置守門員與 CFI 定位校正
