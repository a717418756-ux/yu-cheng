...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.1.6';     // 修返回鍵真正的架構缺陷:原本goPage內呼叫的_pushBackGuard是冪等版本(已有緩衝就不推入)，導致App內部無論導覽幾層(首頁→A→B→C)瀏覽器history深度永遠只有1筆，返回鍵只能「跳回首頁」這一種選擇，完全沒有逐層「上一頁」語意，這才是「返回一次(回首頁)、再返回就離開」的真正原因 (先前幾版修的都是緩衝機制本身的時序bug，此為架構層面的根本缺陷)。修法:新增應用層頁面堆疊_pageStack(不動原本已驗證可靠的單一guard機制)，goPage新增fromBack參數，一般呼叫會推入堆疊，返回鍵觸發的回退不會重複推入；layer10改為逐層pop堆疊回到真正訪問過的上一頁，堆疊耗盡才進入首頁離開流程。用真實history語意(pushState截斷前進紀錄)+實際擷取的app.js程式碼端對端測試驗證：3層深度(list→db→stats)需按3次逐層回到首頁，第4次才觸發離開，全部9種既有情境同時複測全過，與上一版diff僅app.js的+22/-7行，其餘8個檔案零誤傷
