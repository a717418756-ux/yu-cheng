...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.7.1';     // 更新過時的畫面說明文字(非新增說明頁):①本機備份的「備份內容」原只列 exam_data.json/ebooks/media 三項，補上新增的 refbooks/(學習參考書)與 learnmedia/(學習教材影音)，並列出 json 內實際涵蓋的資料 ②本機備份描述改為「全部13項資料」並點出含學習教材 ③雲端備份補上涵蓋範圍說明與「不含大型檔案」提醒 ④匯出JSON標題註明「給程式處理用」，避免與完整備份混淆 ⑤重建條號索引補上使用時機「匯入大量法條後請按一次」與編章節修復 ⑥修正大量匯入答案列的錯誤提示—原寫『DACB 或 1.D 2.A』，但解析器需要題號，純字母DACB實測解析出0個答案，照著打會誤以為功能故障，改為『1.D 2.A 3.C（需帶題號）』。另移除上一版誤加的獨立操作指南(程式碼與樣式皆已清除無死碼)
