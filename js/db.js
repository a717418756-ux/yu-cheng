...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.20.5';    // 兩天改動總檢查:①滑動翻頁會把「在目錄/診斷記錄裡上下捲動」誤判成翻頁，改為只認起點不在面板內、且水平位移大於垂直位移 ②實體按鍵攔截會吃掉輸入框裡的空白鍵與方向鍵，改為在輸入框內不攔截 ③開新書重置翻頁佇列 ④預載判斷改用自己算的頁碼 ⑤移除從未使用的 _readerUIVisible(既有死碼)
