...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.1.3';     // ①修結束測驗後手寫筆跡殘留在統計畫面—showQDone的隱藏清單漏了qink畫布；新增_inkTeardown()統一收尾(清空畫布+退出標註模式+隱藏)，showQDone與exitQ都呼叫，_inkSync時恢復顯示以免返回題目後看不到 ②修「下一題」被擠成兩行:補white-space:nowrap與明確的flex分配(圖示鈕固定42px不壓縮、結束鈕不壓縮、下一題吃剩餘空間)，並明確定義.qfoot容器為nowrap ③題庫/資料庫/法條的chip與hd-btn補上與答題底列一致的按壓縮放回饋(eink改用邊框強調不做縮放)
