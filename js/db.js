...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.20.1';    // 換章後第一下白按(使用者診斷記錄:換到新章後按下一頁頁碼不變):①epub.js 的捲動補償(counter)整個停用——換章已改為自己定位不再需要它，它在新章版面收斂時會把剛捲的一頁推回去 ②章內翻頁捲完後確認頁碼有變，沒變就再捲一次；捲得動只是被推回時不換章，避免跳過本章剩餘頁
