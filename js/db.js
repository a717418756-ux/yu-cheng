...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.10.0';    // 題庫新增「批次修改」:原本批次選取後只能刪除，匯入後要整批修正科目/年度/考試別只能一題一題編輯。新增 openBatchEdit/applyBatchEdit—只填要改的欄位，留空不動；科目欄提供既有科目候選清單避免打錯字產生新分類；★修改後必定重建 searchBlob(公式與 saveQ 完全一致，已驗證欄位相同)，否則改了分類卻搜不到。沿用既有 .ov+.sh 彈窗樣式，函式已入 DataMod 白名單
