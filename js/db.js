...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.1.0';     // 筆記重做:①手寫改為直接疊在題目上圈畫(canvas覆蓋qbody，平時pointer-events:none不擋作答，按✏️才進標註模式)，筆跡存inkImg，下次做到該題直接看到，取代原本另開繪圖板的做法 ②文字備註與題庫「備註/詳解」共用同一note欄位，開啟時讀資料庫最新值、儲存時先取最新再寫回避免覆蓋其他欄位，雙向同步 ③換題自動存上一題筆跡並載入本題 ④清除舊繪圖板全部程式與樣式無殘留。另確認今日複習遺忘曲線(calcNextReview升降級距)與加強複習池(getWrong危險題∪收藏)皆正常運作
