...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.5.1';     // 新增 Azure TTS 用量估算:在兩個實際送出請求處(播放/預抓)各累加一次字元數，設定頁顯示本月用量、佔免費額度百分比與進度條，達80%轉警示色，跨月自動歸零。★明確標示為估算值—SSML標籤是否計入、失敗重試、跨月時區都可能與微軟後台有出入。隔離設計:整個統計函式包在try/catch、非同步不阻塞、getSetting/setSetting失敗皆靜默吞掉，朗讀主流程一行未改，統計壞掉也絕不影響朗讀
