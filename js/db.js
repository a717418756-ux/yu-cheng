...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.19.7';    // 跳頁根因(由使用者診斷記錄確認):epub.js 判斷章內還有沒有下一頁是用「scrollLeft+容器寬+每頁寬<=總寬」，容器寬與每頁寬差幾像素(分欄間距/邊框/裝置四捨五入)就會提早判到章尾→跳過該章最後一頁直接換章；往回同理，章首會空按一次，換章後又停在上一章第一頁(倒退整章)。改用 epub.js 自己算的頁碼判斷:章內還有頁就自己捲一頁(scrollTo+fill+reportLocation)，真的到章界才換章，往回換章後等版面排完再補到上一章最後一頁
