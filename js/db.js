...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.1.2';     // ①修標註位移真因:工具列原為position:sticky且排在題目容器流程最前，開啟標註時佔掉約50px高度把題目整個下推，按完成後題目移回但筆跡留原位→看似位移；改position:fixed脫離版面流程，開關標註完全不影響版面 ②筆跡改為僅在已作答(S.quiz.ans)時顯示，避免重測時上次圈的答案先曝光；公布解答當下同步載入 ③底列改用線性SVG圖示取代emoji(星標/標註/備註/下一題)，統一42px高、圓角與按壓縮放，主次動作分明；同時修星標切換用textContent會清掉SVG圖示的問題，狀態改由CSS .qstar.on 呈現
