...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.15.1';    // 關聯法條改為單一清單(合併正向與反向):原本分成「🔗關聯法條」與「↩被這些引用」兩區，讀法條時多一個區塊要看。改為合併顯示—手動填的＋本條內文引用到的＋引用到本條的，三者去重後同列。反推來源以「法規名稱」呈現並依名稱去重，避免同一部子法有多條引用時擠出一堆重複標籤(例如遴選辦法§1與§3都引用警職法§12，警職法§12只會顯示一個辦法標籤)。已移除用不到的反向專用CSS，無死碼
