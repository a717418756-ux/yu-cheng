...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.3.4';     // 重修成長區朗讀跳句(v4.3.3修法失效):前版用「設旗標→cancel→還原旗標」抑制，但speechSynthesis.cancel() 觸發 onend 是【非同步】的，回呼往往在旗標已還原後才執行，抑制完全沒生效—實測在非同步條件下仍會一路衝到「朗讀完成」。改用與時序無關的世代序號：每次啟動新句子 ++_ttsSeq 並讓該 utterance 記住自己的序號，回呼執行時若序號已被新朗讀取代即放棄前進。調語速、朗讀中點別句都適用。已用非同步 cancel 模擬回歸測試：連調3次語速穩定停在原句、正常唸完自動前進與結束正常、點選別句正確跳轉不亂前進。舊旗標完全移除無死碼
