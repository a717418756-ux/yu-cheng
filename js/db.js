...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.20.2';    // 連跳兩頁(使用者診斷記錄:頁碼沒變再捲一次):epub.js 的 reportLocation() 排到下一個畫面影格才更新頁碼，await 回來讀到的是舊頁碼；4.20.1 據此判斷「沒翻到」又多捲一頁。①章內翻頁改以捲動位置有無變化判斷，移除重捲 ②_epubGoto 每次移動後等兩個影格再讀位置，避免多翻 ③目錄跳轉失敗時操作視窗未結束、守門員永久停擺，改 finally ④守門員拉回後不再硬設錨點(與頁首位置不同會反覆拉扯)
