...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.11.0';    // 新增限時模擬考(只出選擇題):與一般練習的核心差異是「作答時不顯示對錯」，模擬真實考試，交卷後才一次檢討；可選科目、題數(25/40/50/60)、時間(30~120分)。出題池只收 type=mc 且已填答案者(無答案無法計分)，用 Fisher-Yates 洗牌避免 sort(Math.random) 的分佈偏差(實測偏差5%)。倒數計時器 fixed 於頂端、pointer-events:none 不擋作答，剩5分鐘轉警示色並緩慢呼吸；時間到自動交卷。計時器在交卷(showQDone)與中途離開(exitQ)都會清除，不殘留。沿用既有 startQWithPool 流程未重寫答題邏輯。e-ink 版不做動畫避免殘影耗電
