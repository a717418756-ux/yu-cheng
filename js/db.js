...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.12.0';    // 新增法條反向連結(被哪些法條引用):在A的關聯法條填了B之後，開啟B會自動顯示「↩ 被這些引用：A」，不需要在B再填一次。★設計取捨—不另存反向關係，而是顯示時即時推導：①免重複輸入 ②不會有改一邊忘一邊的同步問題 ③既有資料立刻生效不需轉檔。比對規則與 showLawPop 一致(先 §簡寫正規化、再用 art2n 比對條號)，已驗證「§12的引用不會誤配到§120」；引用未指定條號時視為指向整部法規並標示。視覺上與正向關聯區隔(正向紫=我引用誰、反向青=誰引用我)，含 e-ink 虛線框版本
