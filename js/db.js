...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.4.3';     // 修題庫標註模式完全畫不出東西:前版為了「標註時仍能捲動長題目」，改成手指需長按260ms才進入繪製、且 pointermove 一觸發就取消長按。但手指按在螢幕上必然有微小抖動，pointermove 必定觸發，長按永遠被取消→手指完全畫不出來(這是必然失敗的設計，非偶發)。改回一律直接繪製(手指/筆/滑鼠皆同)，CSS touch-action 也改回 none 避免拖曳被當成捲動而斷線；要捲動時關閉標註模式即可—標註是短暫圈畫動作，不該為了捲動犧牲「按下就能畫」的基本預期。長按機制與 _touchHold 變數完全移除，無死碼
