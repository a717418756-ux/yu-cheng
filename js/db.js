...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.16.2';    // 修資料庫搜尋框消失:前一版把下拉與搜尋框包在自訂 div(padding:0 12px)內，但裡面的 .sb 本身已有 margin:8px 12px，左右邊距疊加成24px；且 .sb .ic 是 position:absolute;left:10px 會壓在下拉選單上，input 又保留 padding-left:33px 讓位給該圖示 —— 三者疊加把輸入框擠到看不見。改為直接用既有 .sb 容器並加 .sb-with-sel 修飾類:放大鏡改靜態定位排在下拉之後、input 縮排改為10px、下拉固定86px寬。手機390px下輸入框仍有250px可用。新樣式只作用於 .sb.sb-with-sel，其他頁面的搜尋框完全不受影響
