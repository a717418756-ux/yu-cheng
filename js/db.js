...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.7.2';     // 更新過時的畫面說明(前版修改因容器問題未進出貨檔，本次重做並交叉驗證):①雲端「備份範圍」原寫「題庫+法條」，實際已涵蓋9項—改為完整列出(題庫含手寫標註與複習進度、法條、答題記錄、設定、倒數日、使用統計、英語教材、單字本、健康數據)並註明不含大型檔案 ②本機「備份內容」原只列 exam_data.json/ebooks/media 三項，實際有4個資料夾—補上 refbooks/(學習參考書)與 learnmedia/(學習教材影音)，並列出 json 內實際涵蓋的9項資料 ③本機備份描述補上學習參考書與學習教材 ④匯出JSON標題註明「非完整備份」避免混淆。已用程式碼交叉驗證：說明列出的項目與 gdriveBackup/localBackup 實際讀取的 store 完全一致
