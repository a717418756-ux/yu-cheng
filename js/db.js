...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.15.2';    // 查官網原文兩項調整:①位置從「每條法條下方」移到「資料庫第一層每一筆法規」，點🏛直接開整部法規，不必先進去再按(事件有 stopPropagation，不會同時觸發進入法規) ②fallback 不再導向 Google—改用官網自己的搜尋結果頁 law.moj.gov.tw/Law/LawSearchResult.aspx?ty=LAW&kw=法規名，省掉「先看Google結果再點全國法規」那一步，所有情況都直接落在官網。條文層的舊按鈕與樣式已移除無死碼。註:自動關聯連結本來就沒有按鈕，開啟法規時即時計算，新增法規後立即生效
