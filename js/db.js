...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.1.9';     // 修法規搜尋框「§條號」搜尋的兩個實質問題(承4.1.8的§支援):①原正則只收(\d+)，輸入「警察法§2-1」子條號整個不匹配而靜默退回全文搜尋，改收(\d+(?:[-－之]\d+)?) ②原比對 String(l.articleNumber)===kwArt 永遠不成立(articleNumber是2000格式、kwArt是"2")，實際只靠字串includes勉強運作，而字串比對又讓「§2」誤匹配到「第2條之1」(article含「第2條」字樣)；改為統一用art2n()把輸入換算成與articleNumber同一套編碼(主號*1000+子號)再精確比對。組字串時子號須接在「條」之後成「第X條之Y」，art2n的子條號規則為/條之(N)/，寫成「第2之1條」解析不出。驗證:§2只回第2條不再誤含之1、§2-1與§2之1皆正確定位、原有純數字與空格寫法不受影響
