...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.20.0';    // 電子書換章改用專業閱讀器作法:不再用 epub.js 的 next()/prev() 把相鄰章節接在前後、靠捲動補償對齊(補償量在版面排完前就算好，落點才會飄)。改為直接顯示目標章再定位(往後→第1頁、往回→最後一頁)，全程畫面隱藏，排好才顯示。四種結構實測共174頁往返，零重複零跳過、完全對稱
