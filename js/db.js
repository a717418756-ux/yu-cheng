...
// 版本常數
// ════════════════════════════════════════════════════════════════
const APP_VERSION  = '4.4.1';     // 漏網掃描修正:①showLawPop 的條號比對缺少 art2n fallback—renderDB/openLawGroup 都已加上「articleNumber 未建立時即時換算」，唯獨此處漏了，導致剛匯入、尚未按過「重建條號索引」的法條點連結會查無資料 ②tts.js 的 _speakWithSystem(Azure失敗後的系統語音 fallback 路徑)其 onerror完全沒有防護，程式主動 cancel(停止/切換聲音/調語速)觸發時會直接 idx++ 憑空跳過一段，且未檢查 _TTS.speaking；比照另一條路徑補上 interrupted/canceled 判斷，並將兩處防護統一(另一處原本也缺 speaking 檢查)。其餘掃描項目均通過：9JS語法/CSS括號、白名單零風險、死碼與已移除機制零殘留、新增函式全部有使用
